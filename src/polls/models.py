...
class QuestionModel(PublicationModel):
    ...
    poll = models.ForeignKey(PollsModel, on_delete=models.CASCADE, verbose_name='Опрос')
...
class AnswerModel(models.Model):
    """Model for answers"""
    user = models.ForeignKey(User, on_delete=models.DO_NOTHING, verbose_name='Пользователь', default=AnonymousUser)
    question = models.ForeignKey(QuestionModel, on_delete=models.DO_NOTHING, verbose_name='Вопрос')
    choice = models.ForeignKey(ChoiceModel, on_delete=models.DO_NOTHING, verbose_name='Ответ')
    data_created = models.DateField(auto_now_add=True, verbose_name='Дата создания')
